---
name: dbs-hook
description: "短视频/文章开头优化。5Phase工作流+3种方法(素材提取/增补/悬念制造)+开头公式(话题+Hook+可信度)+口播友好检查。触发词：/dbs-hook、帮我优化开头、开头怎么写、hook、吸引人、开头不行、前5秒。不适用场景：正文诊断→dbs-content、结尾优化→dbs-content、标题优化→dbs-xhs-title。正例：'帮我看看这个短视频开头行不行'→触发; '开头怎么改才能留人'→触发。反例：'帮我诊断整个文案'→不触发→dbs-content; '结尾怎么写'→不触发→dbs-content。"
...
| 失败模式 | 原因 | 兜底动作 |
|---------|------|---------|
| 内容本身无价值，开头再优化也没用 | 跳过Phase 2内容质量诊断 | 回到Phase 2先诊断内容质量，若内容无价值则告知用户先完善内容 |
| 优化后开头仍然不留人 | 未制造悬念或开头=答案 | 检查开头是否直接给了答案→改为悬念形式；检查是否自问自答→改为直接陈述 |
| 优化后开头不适合口播 | 未做口播友好检查 | 念一遍优化后的开头，标记拗口处重写 |
